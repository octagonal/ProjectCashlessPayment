    ...
    class RegisterVM : ObservableObject, IPage
    {
        ...
        private async void SaveRegister()
        {
            ...
            if (SelectedRegister.ID == 0)
            {
                ...
                {
                    client.SetBearerToken(ApplicationVM.token.AccessToken);
                    HttpResponseMessage response = await client.PostAsync(lib.Constants.WEBURL + "api/Register", new StringContent(input, Encoding.UTF8, "application/json"));
                    if (response.IsSuccessStatusCode)
                    {
            ...
            else
            {
                ...
                {
                    client.SetBearerToken(ApplicationVM.token.AccessToken);
                    HttpResponseMessage response = await client.PutAsync(lib.Constants.WEBURL + "api/Register", new StringContent(input, Encoding.UTF8, "application/json"));
                    if (!response.IsSuccessStatusCode)
                    {
                        Console.WriteLine("error");
                    }
        ...
        private async void DeleteRegister()
        {
            ...
            {
                client.SetBearerToken(ApplicationVM.token.AccessToken);
                HttpResponseMessage response = await client.DeleteAsync(lib.Constants.WEBURL + "api/Register/" + SelectedRegister.ID);
                if (!response.IsSuccessStatusCode)
                {
                    Console.WriteLine("error");
                }
